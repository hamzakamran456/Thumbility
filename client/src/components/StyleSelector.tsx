...
const StyleSelector = ({
  ...
  const styleDescriptions: Record<ThumbnailStyle, string> = {
    "Bold & Graphic": "High contrast, bold typograpghy, striking visuals",
  ...
  return (
    <div className="relative space-y-3 dark">
      ...
      <button
        ...
        className="w-full flex items-center justify-between rounded-md border px-4 py-3 text-left transition bg-white/8 border-white/10 text-zinc-200 hover:bg-white/12"
      >
        ...
        <div className="absolute bottom-0 z-50 mt-1 w-full rounded-md border border-white/12 bg-black/20 backdrop-blur-3xl shadow-lg">
          {thumbnailStyles.map((style) => (
            <button
              key={style}
              type="button"
              className="w-full items-start gap-3 px-4 py-3 text-left transition hover:bg-black/30"
              onClick={() => {
                onChange(style);
                setIsOpen(false);
              }}
