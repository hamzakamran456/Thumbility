...
const MyGeneration = () => {
  ...
  const aspectRadioClassMap: Record<string, string> = {
    "16:9": "aspect-video",
    "1:1": "aspect-square",
    "9:16": "aspect-[9/16]",
  };
  ...
  const hanldeDownload = (image_url: string) => {
        ...
        {!loading && thumbnails.length === 0 && (
          <div className="text-center py-24">
            ...
            <p className="text-sm text-zinc-400 mt-2">
              Generate first thumbnail to see it here
            </p>
          </div>
        )}
        {/* Grid */}
        {!loading && thumbnails.length > 0 && (
          <div className="columns-1 sm:columns-2 lg:columns-3 2xl:columns-4 gap-8">
            {thumbnails.map((thumb: IThumbnail) => {
              const aspectClass =
                aspectRadioClassMap[thumb.aspect_ratio || "16:9"];
              return (
                <div
                  key={thumb._id}
